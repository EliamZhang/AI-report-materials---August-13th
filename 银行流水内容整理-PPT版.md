...
- **职责**：识别退票、撤销、失败扣款和直接扣款退回，标记为退票类交易；
- **机制**：明确的交易文本规则，无需实时 AI；质检发现新退票描述时补充为新的识别规则。
...
特例处理：Credit Corp 按子产品线索判断为 BNPL、循环额度或个人贷款；Cash Converters 含门店、终端、卡尾号等消费证据时归为零售，含贷款合同编号时保留贷款含义。

BNPL 知识库维护各提供方限额假设，不用于判断单笔交易是否为 BNPL，而是用于后续负债汇总和额度分析。

### 6. All Other Credit Engine：未识别入账兜底

- **职责**：归集未被收入、转账和负债引擎识别的入账，避免遗漏可识别入账；
- **价值**：作为待优化样本，某类交易频繁落入即说明相关知识库仍有补充空间。

### 7. Fee Engine：识别银行和交易费用

- **职责**：识别 ATM 费、账户费、国际交易费、透支费、退票费和现金预支费；
- **机制**：费用描述 + 交易模式规则，在流水线最后完成归类；新增费用类型可通过评估闭环补充到规则中。
