...
> 示意流水（多笔，含贷款还款、消费、订阅、工资等场景）；金额为模拟示意值。

| 日期 | 金额 | 方向 | 交易描述 |
| --- | --- | --- | --- |
| 2026/03/26 | A$156 | Debit | `BILL PAY Fair Go Finance DT.58oa72 SACC 239` |
| 2026/03/28 | A$12.40 | Debit | `MCDONALDS BRISBANE` |
| 2026/04/02 | A$35.90 | Debit | `CHEMIST WAREHOUSE 123` |
| 2026/04/08 | A$8.99 | Debit | `AMZNPRIMEA*` |
| 2026/04/10 | A$3,420.50 | Credit | `PAYROLL ACME PTY LTD` |
| 2026/04/15 | A$25.00 | Debit | `EFTPOS DEBIT 20NOV LMF GAMES & AMUSEMENTS NORTHMEAD NSW` |
...
系统将原始交易文本加工为标准化的结构化数据。以下为同一批输入流水的识别结果，与上方原始流水逐行对应。

| 日期 | 交易描述 | 标准交易对手 | 主体类型 | 一级分类 | 二级分类 | 交易性质 | 交易金额 | 分类依据 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026/03/26 | `BILL PAY Fair Go Finance DT.58oa72 SACC 239` | Fair Go Finance | 贷款机构 / 小额信贷机构 | 贷款及还款 | SACC Loans | 贷款还款 | A$156 | 交易方向、机构名称、产品关键词及历史交易模式 |
| 2026/03/28 | `MCDONALDS BRISBANE` | McDonald's | 商户企业 | 消费 | Dining Out | 消费支出 | A$12.40 | 商户知识库标准名匹配 |
| 2026/04/02 | `CHEMIST WAREHOUSE 123` | Chemist Warehouse | 商户企业 | 消费 | Health | 消费支出 | A$35.90 | 商户知识库标准名匹配 |
| 2026/04/08 | `AMZNPRIMEA*` | Amazon Prime | 商户企业 | 订阅 | Subscription TV | 订阅支出 | A$8.99 | 商户知识库标准名匹配 |
| 2026/04/10 | `PAYROLL ACME PTY LTD` | ACME PTY LTD | 雇主 / 收入付款方 | 收入 | salary_payg | 工资收入 | A$3,420.50 | 工资文本模式 + 双周周期 + 稳定金额 |
| 2026/04/15 | `EFTPOS DEBIT 20NOV LMF GAMES & AMUSEMENTS NORTHMEAD NSW` | LMF Games & Amusements | 商户企业 | 消费 | Entertainment | 消费支出 | A$25.00 | 商户知识库标准名匹配 |
...
| 预计待还金额   | A$564           |
| 还款频率       | 每两周 A$156    |
| 月还款金额（估算） | 约 A$338（A$156 × 26/12） |
| 预计剩余期数（估算） | 约 4 期（借款 ÷ 每期 ≈ 10 期，已还约 6 期） |
| 90 天还款次数  | 6 次            |
| 当前状态       | 在贷            |
| 扣款失败       | 否              |
| 还款中断       | 否              |
...
本次试点形成了从数据源选择、变量生产、变量筛选、模型应用到效果验证的完整链路。平台共产出68 个经筛选可用变量，其中19 个进入新版交易模型。最终效果如下：
...
引入Agent变量后，AUC由0.643提升至0.648，KS由0.209提升至0.214，二者均提升0.005，即0.5 个百分点，说明模型整体区分度和排序能力均得到改善。
...
因此，墨西哥试点的成绩可以完整概括为：选择短信、APPLIST和借还款行为三类数据源，产出68 个经筛选可用变量，19 个变量进入新版模型，最终带来AUC、KS和Top10% Lift提升。
...
|资源项|当前投入|计划投入|说明|
| --- | --- | --- | --- |
|模型团队人力|0.3 人／月|0.3 人／月|当前投入与计划一致|
|算法平台团队人力|0.5 人／月|0.5 人／月|当前投入与预算一致|
